...
const MyOrderDetails = () => {
  ...
  const generateInvoice = (order, action) => {
    if (!order) {
    ...
    doc.text(`Customer: ${order.shippingAddress.name}`, 14, 32);
    doc.text(`Address: ${order.shippingAddress.address1}, ${order.shippingAddress.city}`, 14, 39);
    doc.text(`Pincode: ${order.shippingAddress.pincode}`, 14, 46);
    doc.text(`Contact: ${order.shippingAddress.mobile}`, 14, 53);
    doc.text(`Status: ${order.status}`, 14, 60);
  
    // Table for Cart Items
    autoTable(doc, {
      startY: 70,
      head: [["Sr. No", "Product", "Price", "Quantity", "Total"]],
      body: order.cartItems.map((item, index) => [
        index + 1,
        item.title,
        `₹${item.price}`,
        item.qty,
  ...
  <button
    ...
    onClick={() => generateInvoice(order, "view")}
  >
    View Invoice
  </button>
  <button
  ...
  >
    Print Invoice
      ...
      <div className="mt-4">
        ...
        <p className="text-gray-600">
          Name: {order.shippingAddress.name},<br />
          Address: {order.shippingAddress.address1}, {order.shippingAddress.city}<br />
          Pincode: {order.shippingAddress.pincode}, Contact: {order.shippingAddress.mobile}
        </p>
      </div>
